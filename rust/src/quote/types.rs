        ...
        match r#type {
            ...
            WarrantType::Call | WarrantType::Put => Ok(Self {
                ...
                expiry_date: parse_date(&info.expiry_date)
                    .map_err(|err| Error::parse_field_error("expiry_date", err))?,
                strike_price: info.last_done.parse().ok(),
                ...
                itm_otm: info.last_done.parse().ok(),
                implied_volatility: info.last_done.parse().ok(),
                delta: info.last_done.parse().ok(),
                call_price: info.call_price.parse().ok(),
                to_call_price: info.to_call_price.parse().ok(),
                effective_leverage: info.last_done.parse().ok(),
